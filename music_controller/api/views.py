...
from .serializers import RoomSeriallizer, CreateRoomSerializer,\
    UpdateRoomSerializer


class RoomView(generics.ListAPIView):
    queryset = Room.objects.all()
    serializer_class = RoomSeriallizer
...
class GetRoom(APIView):
    ...
    def get(self, request, format=None):
        ...
        if code is not None:
            ...
            if len(room) > 0:
                room_obj = room.first()
                data = self.serializer_class(room_obj).data
                data['is_host'] =\
                    self.request.session.session_key == room_obj.host
                return Response(data, status=status.HTTP_200_OK)
            return Response({'Room Not Found': 'Invalid Room Code'},
                            status=status.HTTP_404_NOT_FOUND)
        return Response({'Bad request': 'Code parameter is not in request'},
...
class JoinRoom(APIView):
    ...
    def post(self, request, format=None):
        ...
        if code:
            room_q = Room.objects.filter(code=code)
            if len(room_q) > 0:
                # room = room_q.first()
...
class CreateRoomView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            return Response(RoomSeriallizer(room).data,
                            status=status.HTTP_200_OK)
